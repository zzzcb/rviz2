...
AssimpLoader::AssimpLoader()
{
	...
	importer_->SetIOHandler(new ResourceIOSystem()); //TODO
...
Ogre::MeshPtr AssimpLoader::meshFromAssimpScene(const std::string& name, const aiScene* scene)
{
    if (!scene->HasMeshes()) 
    {
        std::cout << "No meshes found in file [" << name.c_str() << "]" << std::endl;
        return Ogre::MeshPtr();
    }
 ...
 */
std::vector<Ogre::MaterialPtr> AssimpLoader::loadMaterials(const std::string& resource_path, const aiScene* scene)
{
	...
	if (!Ogre::TextureManager::getSingleton().resourceExists(resource_path, ROS_PACKAGE_NAME)) {
		...
		if (res.size != 0) {
			...
			try {
                ...
                std::cout << "Could not load texture [" << resource_path.c_str() << "]: " << e.what() << std::endl;
			}
		}
	}
}
